...
export default function LoginSettingPage(_props: Route.ComponentProps) {
  ...
  const [form] = useForm({
    id: "login-setting-form",
    lastResult:
      fetcher.data?.intent === "updateLoginSetting" ? fetcher.data : undefined,
    shouldValidate: "onSubmit",
  });

  fetcher.register("updateLoginSetting", {
    onSuccess: () => toast.success("ログインページ設定を保存しました"),
    onHandlerError: ({ error }) =>
      toast.error(error?.[""]?.[0] ?? "保存に失敗しました"),
  });

  const isPending = fetcher.isPending("updateLoginSetting");
  ...
  return (
    <section>
      ...
      <fetcher.Form
        ...
        {...getFormProps(form)}
      >
        <input type="hidden" name="intent" value="updateLoginSetting" />

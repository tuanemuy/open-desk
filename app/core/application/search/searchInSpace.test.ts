...
import { searchInSpace } from "./searchInSpace";
...
describe("searchInSpace", () => {
  ...
  it("should return search results with valid spaceId and keyword", async () => {
    ...
    const result = await searchInSpace({
      container,
  ...
  it("should target all 4 source types (RECORD, COMMENT, THREAD, FILE) when sourceTypes is empty", async () => {
    const container = getContainer();
    ...
    expect(calledQuery.scope.type).toBe("SPACE");
  });
  ...
  it("should return only THREAD results when sourceTypes is ['THREAD']", async () => {
    const container = getContainer();
    ...
    const calledQuery = searchSpy.mock.calls[0][0];
    expect(calledQuery.filters.sourceTypes).toEqual(["THREAD"]);
  });

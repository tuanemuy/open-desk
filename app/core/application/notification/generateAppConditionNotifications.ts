...
export type GenerateAppConditionNotificationsInput = {
...
export async function generateAppConditionNotifications({
  container,
  input,
}: ServiceArgs<GenerateAppConditionNotificationsInput>): Promise<GenerateNotificationsOutput> {
  const senderId = input.senderId as UserId;
  const recipientIds = input.recipientIds.map((id) => id as UserId);
  const excludeUserIds = (input.mentionedUserIds ?? []).map(
    (id) => id as UserId,
  );

  const { notifications } =
    NotificationGenerationService.generateAppConditionNotifications({
      senderId,

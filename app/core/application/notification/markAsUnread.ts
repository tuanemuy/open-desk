...
import type { NotificationStatusOutput } from "./dto";

export type MarkAsUnreadInput = {
...
export async function markAsUnread({
  container,
  input,
}: ServiceArgs<MarkAsUnreadInput>): Promise<NotificationStatusOutput> {
  ...
  return await container.unitOfWorkProvider.transaction(async (ctx) => {
    ...
    const { entity: updated } = Notification.markAsUnread(notification);
    await ctx.notificationRepository.save(updated);
    return { notificationId: updated.notificationId, isRead: updated.isRead };
  });
}

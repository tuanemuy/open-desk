...
export type GenerateSpaceNotificationsInput = {
...
export async function generateSpaceNotifications({
  container,
  input,
}: ServiceArgs<GenerateSpaceNotificationsInput>): Promise<GenerateNotificationsOutput> {
  const senderId = input.senderId as UserId;
  const recipientIds = input.recipientIds.map((id) => id as UserId);

  const { notifications } =
    NotificationGenerationService.generateSpaceNotifications({
      senderId,

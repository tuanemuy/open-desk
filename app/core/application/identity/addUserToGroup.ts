...
import type { GroupMembershipOutput } from "./dto";
...
export async function addUserToGroup({
  container,
  input,
}: ServiceArgs<AddUserToGroupInput>): Promise<GroupMembershipOutput> {
  ...
  await container.unitOfWorkProvider.transaction(async (ctx) => {
    ...
    await ctx.membershipRepository.addUserToGroup({ userId, groupId });
  });

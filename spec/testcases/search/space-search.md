...
|---|---|---|---|
| スペース内に検索対象のデータが存在する | 有効な spaceId と keyword でスペース内検索を実行する | アクセス権のある検索結果が返る | |
| スペース内に検索対象のデータが存在する | sourceTypes を空配列で検索する | 4種別（RECORD, COMMENT, THREAD, FILE）すべてが対象となる | |
| スペース内に検索対象のデータが存在する | sourceTypes に RECORD のみ指定して検索する | RECORD 種別のみの検索結果が返る | |
| スペース内に検索対象のデータが存在する | sourceTypes に THREAD のみ指定して検索する | THREAD 種別のみの検索結果が返る | |
...
| 検索エンジンが障害中 | スペース内検索を実行する | SearchExecutionError が返る | |
...
| 検索結果にアクセス権のない項目が含まれる | スペース内検索を実行する | アクセス権のない項目がフィルタリングされた結果が返る | |
| スペース内に検索対象のデータが存在しない | 有効な keyword で検索する | 空配列と totalCount: 0 が返る | |
